...
| **Muse — Builder** | `claude-sonnet-4-7` | `claude-opus-4-7` | 98% Opus quality at fraction of cost (per RAP-150 issue body); cost-conscious by default for routine plan execution via `forge build`. Opus escalation when Sonnet plan output is rejected at a checkpoint review (per [`forge/entities.md § Builder mode`](../../current/forge/entities.md)). |
