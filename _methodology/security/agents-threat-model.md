...
- **No live threat exceeds RPN 10** after the 2026-05-26 batch. **AT-07 dropped to RPN 4** (regex scrub middleware shipped 2026-05-26). **AT-08 dropped to RPN 4** (five-layer auth chain audited 2026-05-26; was already in production via the `add-tg-canvas-surface` shipment but unverified in this document). **AT-12 dropped to RPN 4** (pre-flight path-scope validator shipped 2026-05-26). The remaining elevated rows are AT-06 (RPN 10, force-push to main) and AT-10 (RPN 10, accepted risk gated on cross-repo proposal). No threat breaches the blocking threshold.
