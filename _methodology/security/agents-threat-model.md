...
| AT-07 | **Prompt-builder secret redaction.** Regex-strip known-secret patterns (Anthropic `sk-ant-...`, GitHub `ghp_...` / `gho_...` / `ghs_...`, Linear `lin_api_...`, JWT three-segment shape, hex tokens ≥32 chars) before any outbound Anthropic request. Replace with `[REDACTED-<pattern-name>]`. | Scrub middleware in Forge's `callClaude` + cli-core-hosted Proxy wrap for Muse / Maestro Anthropic clients | **Shipped (both halves)** — Forge half: [`openspec/archive/2026-05-26-add-forge-prompt-secret-redaction/`](../../archive/2026-05-26-add-forge-prompt-secret-redaction/); Muse + Maestro half: [`openspec/archive/2026-05-26-add-muse-prompt-secret-redaction/`](../../archive/2026-05-26-add-muse-prompt-secret-redaction/) (closes Q-FPSR-3). AT-07 post-mitigation L=1, RPN=4. Pattern set + Proxy helper lifted to `@rapoport-studio/cli-core/security` per rule-of-three (Forge + Muse + queued cli-core/llm consumer). Event kind `prompt.secret_redaction` in `cli-core/src/events/`. Muse chokepoint: `packages/muse/src/shared/client.ts § createClient` wraps every Anthropic client; Maestro headless wraps the dynamically-imported SDK client identically. |
