...
| AT-09 | **Reframed 2026-05-26.** The original wording assumed a clock-expiring session-token model; the actual mechanism in production is the `tg_user_bindings` row created via the signed-JWT-in-DM consume flow (D-TCS-6) — bindings are persistent until revoked, not clock-expiring. The signed invite JWT itself is one-shot with a 24h TTL (per `telegram.md § Identity binding`), so the "token-replay" subset of this threat is closed by design (consumed tokens cannot be re-used). The remaining residual risk is `tg_user_bindings` table-level revocation: a stale binding (e.g. a Telegram account compromised or rotated to a different user) currently has no scheduled revocation. Mitigation: surface a `Revoke Telegram binding` action in the studio canvas UI that calls `DELETE FROM tg_user_bindings WHERE …` for the binding row. | Studio canvas UI action (deferred) | **Open** — needs UI + RPC for binding revocation; scaffold as `add-telegram-binding-revoke-action` follow-up. RPN-as-stated already accepts this gap (L=2, RPN=8). |
