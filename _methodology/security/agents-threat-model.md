...
| AT-16 | **Web-fetch egress-then-ingest injection** — `fetch_url` / `web_search` return attacker-controlled page/search content that re-enters an agent prompt as trusted text; an injected instruction in a fetched page or search result steers an agent (F-8). _Mitigated: `<untrusted_input>` wrapping on both tool results shipped via [`openspec/changes/close-untrusted-input-dispatch-chain/`](../../changes/close-untrusted-input-dispatch-chain/) — pre-mitigation L=3/I=4/RPN=12._ | Muse foundation tools (`packages/muse/src/tools/{fetch_url,web_search}.ts`) | ATLAS: Initial Access | 1 | 4 | **4** |
| AT-17 | **MCP-spawn credential inheritance → unattended dispatch** — the Forge MCP server spawned its CLI with full `process.env`, handing every secret (`ANTHROPIC_API_KEY`, `GITHUB_BOT_TOKEN`, `LINEAR_BOT_API_KEY`, Supabase service-role) to a process that, via `forge_dispatch`, runs unattended `--auto-push`/`--auto-pr` builds; chains with AT-01/AT-16 (injected instruction → credentialed auto-pushed build) (F-2). _Mitigated: allowlist-derived `buildSpawnEnv` replaces `env: process.env` at `spawn.ts`/`dispatch.ts` (fail-closed) + input wrapping (AT-16), shipped via [`openspec/changes/close-untrusted-input-dispatch-chain/`](../../changes/close-untrusted-input-dispatch-chain/) — pre-mitigation L=2/I=5/RPN=10. Residual (`--auto-*` confirmation gating) tracked as `Q-UID-1`._ | Forge MCP server spawn (`packages/forge/src/mcp-server/{spawn,tools/dispatch}.ts`) | I + E (STRIDE) | 1 | 5 | **5** |
